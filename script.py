...
def pedidosporcliente():
    selectconsulta = f""" select c.nome_cli, count(*) from clientes c left join pedidos p on c.id_cliente = p.id_cliente group by c.nome_cli 
    order by count desc;"""
    lista = execute(selectconsulta)

    for linha in lista:
            print(
                f" Nome do Cliente: {linha[0]}  Quantidade de Pedido: {linha[1]} "
            )
    return lista
def produtosporfornecedor():

    selectconsulta = f"""select f.nome_empresa, count(p.id_produto) from fornecedores f 
    left join fornecedores_has_produtos fhp on f.id_fornecedor = fhp.id_fornecedor 
    left join produtos p on p.id_produto = fhp.id_produto group by f.nome_empresa order by count desc; """
    
    lista = execute(selectconsulta)

    for linha in lista:
            print(
                f" Nome do fornecedor: {linha[0]}  Quantidade de produto: {linha[1]} "
            )
    return lista

def totalclientecidade():

    selectconsulta = f"""select e.cidade ,count(c.id_cliente) from enderecos e left join clientes c 
    on e.id_endereco = c.id_cliente group by e.cidade order by count desc ;"""
    
    lista = execute(selectconsulta)

    for linha in lista:
            print(
                f" Cidade: {linha[0]}  Quantidade de Cliente: {linha[1]} "
            )
    return lista
...
def menu():
    ...
    print("-----------Relatórios-----------------")
    print("21. Total de pedidos por cliente")
    print("22. Total de produtos por fornecedor")
    print("23. Total de Clientes por cidade")
    print("0. Sair")
    print("\n")
...
def tratamentodeopcoes(opcao):
    ...
    if opcao == "21":
       total = pedidosporcliente()
       return total

    if opcao == "22":
       total = produtosporfornecedor()
       return total

    if opcao == "23":
        total = totalclientecidade()
        return total
